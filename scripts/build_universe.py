            ...
            for col in t.columns:
                ...
                if "company" in cl:
                    ...
                    for col2 in t.columns:
                        ...
                        if 1 < len(sample) < 8 and sample.isalpha():
                            tickers = [
                                f"{s}.L"
...
def get_euro_stoxx600() -> list[str]:
    ...
    logger.info("Europa total: %d tickers", len(all_tickers))
    return all_tickers
...
def get_additional_asia() -> list[str]:
    ...
    all_t = india + korea + australia + taiwan + (singapore)
    logger.info("Asia adicional: %d tickers", len(all_t))
    return all_t
...
def get_mena_africa() -> list[str]:
    ...
    all_t = saudi + uae + south_africa + israel + turkey
