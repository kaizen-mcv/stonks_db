...
def load_tickers() -> list[str]:
    ...
    if not path.exists():
        logger.error(
            "Ejecuta primero: python scripts/"
            "build_universe.py"
        )
        sys.exit(1)
    with open(path) as f:
...
def get_existing_tickers() -> set[str]:
    ...
    existing = {
        c.ticker for c in
        session.query(Company.ticker).all()
    }
    session.close()
    return existing
...
def main() -> None:
    ...
    for i, ticker in enumerate(tickers, 1):
        logger.info(
            "[%d/%d] %s...", i, total, ticker
        )
        try:
            company_id = fetcher.fetch_company_info(
                ticker
            )
            if company_id:
        ...
        if i % args.batch_size == 0:
            logger.info(
                "--- Progreso: %d/%d "
                "(ok=%d, err=%d) ---",
                i, total, success, errors,
            )

    logger.info(
        "=== Completado: %d ok, %d errores "
        "de %d total ===",
