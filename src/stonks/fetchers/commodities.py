...
COMMODITIES = [
    # Metales preciosos
    ("GOLD", "Gold", "precious_metal", None, "troy_oz", "GC=F"),
    ("SILVER", "Silver", "precious_metal", None, "troy_oz", "SI=F"),
    ("PLATINUM", "Platinum", "precious_metal", None, "troy_oz", "PL=F"),
    ("PALLADIUM", "Palladium", "precious_metal", None, "troy_oz", "PA=F"),
    # Energía
    ("WTI", "Crude Oil WTI", "energy", "oil", "barrel", "CL=F"),
    ("BRENT", "Crude Oil Brent", "energy", "oil", "barrel", "BZ=F"),
    ("NATGAS", "Natural Gas", "energy", "gas", "mmbtu", "NG=F"),
    ("HEATING_OIL", "Heating Oil", "energy", "oil", "gallon", "HO=F"),
    # Agricultura
    ("WHEAT", "Wheat", "agriculture", "grains", "bushel", "ZW=F"),
    ("CORN", "Corn", "agriculture", "grains", "bushel", "ZC=F"),
    ("SOYBEAN", "Soybeans", "agriculture", "grains", "bushel", "ZS=F"),
    ("COFFEE", "Coffee", "agriculture", "softs", "pound", "KC=F"),
    ("COCOA", "Cocoa", "agriculture", "softs", "metric_ton", "CC=F"),
    ("SUGAR", "Sugar #11", "agriculture", "softs", "pound", "SB=F"),
...
class CommodityFetcher(BaseFetcher):
    ...
    def seed_commodities(self) -> int:
        ...
        count = 0
        try:
            for code, name, cat, subcat, unit, yticker in COMMODITIES:
                if session.query(Commodity).filter_by(code=code).first():
                    continue
                session.add(
                    Commodity(
                        code=code,
    ...
    def fetch_prices(
        ...
        try:
            ...
            if code:
                commodities = [
                    session.query(Commodity).filter_by(code=code).first()
                ]
            else:
