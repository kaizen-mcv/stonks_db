...
class FundamentalsFetcher(BaseFetcher):
            ...
            if bal is not None and not bal.empty:
                for col in bal.columns:
                    ...
                    session.add(BalanceSheet(
                        ...
                        total_current_assets=gb(
                            "Current Assets"
                        ),
                        property_plant_equipment=gb(
                            "Net PPE"
                        ),
                        goodwill=gb("Goodwill"),
                        intangible_assets=gb(
    ...
    def fetch_splits(
        ...
        try:
            ...
            session.rollback()
            logger.error(
                "Error splits %s: %s", ticker, e
            )
        finally:
            session.close()
        return stats

    def fetch_all_for_company(
        self, ticker: str
    ) -> dict[str, dict]:
    ...
    def fetch_batch(
        ...
        for i, ticker in enumerate(tickers, 1):
            ...
            results[ticker] = (
                self.fetch_all_for_company(ticker)
            )
        return results
