...
class CoinGeckoFetcher(BaseFetcher):
    ...
    def seed_coins(self) -> int:
        ...
        count = 0
        try:
            for (cg_id, symbol, name, cat) in (
                TOP_COINS
            ):
                if session.query(Coin).filter_by(
                    coingecko_id=cg_id
                ).first():
                    continue
                session.add(Coin(
    ...
    def fetch_prices(
        ...
        try:
            ...
            for coin in coins:
                ...
                vol_map = {
                    int(v[0]): v[1]
                    for v in volumes if v[1]
                }
                mcap_map = {
                    int(m[0]): m[1]
                    for m in mcaps if m[1]
                }
