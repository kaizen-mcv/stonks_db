...
class ECBForexFetcher(BaseFetcher):
    ...
    def _ensure_pairs(self, session) -> dict[str, int]:
        ...
        existing_currencies = {
            c.code for c in session.query(Currency.code).all()
        }
    ...
    def fetch_rates(self, full_history: bool = False) -> dict[str, int]:
        ...
        """
        url = self.HISTORY_URL if full_history else self.RECENT_URL
        ...
        try:
            ...
            logger.info(
                ...
                "completo" if full_history else "90 días",
            )
            self._rate_limit()
            ...
            for cube_time in root.findall(".//ecb:Cube[@time]", NS):
                ...
                for cube_rate in cube_time.findall("ecb:Cube[@currency]", NS):
